...
import com.ameron32.apps.projectbanditv3.activity.GatewayActivity;
...
public class NewMessageNotification {
  ...
  /**
   * Activity that pressing the NOTIFICATION opens
   */
  // FIXME: is this still accurate?
	private static final Class NOTIFICATION_OPEN_ACTIVITY = GatewayActivity.class;
...
//		exampleItem.append("   Example content");
		...
		final String messageCount = messages.length + " messages.";
		
		final String ticker = messageCount;
...
//     .setLargeIcon(picture)
    ...
    .setContentIntent(PendingIntent.getActivity(context, 0, new Intent(context, NOTIFICATION_OPEN_ACTIVITY), PendingIntent.FLAG_UPDATE_CURRENT))

				// Show an expanded list of items on devices running Android 4.1
				// or later.
    .setStyle(inboxMessages(messages))
  ...
  private static InboxStyle inboxMessages(
      String[] messages) {
    ...
    inboxStyle.setBigContentTitle("New Messages").setSummaryText(messages.length + " new messages.");
    return inboxStyle;
  }
