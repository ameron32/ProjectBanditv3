...
public class FloatingHintTextView
  ...
  public FloatingHintTextView(
    ...
    normalHintColors = getResources().getColorStateList(R.color.floating_hint_text);
  ...
  @Override protected void onTextChanged(
    ...
    if (isEmpty) {
      ...
      // The TextView will show a hint since the field is empty, but since we're
      // animating
      // from the floating hint, we don't want the normal hint to appear yet. We
      // set it to
      // transparent here, then restore the hint color after the animation has
      // finished.
  ...
  @Override protected void onDraw(
    ...
    if (TextUtils.isEmpty(getHint())) { return; }
    
    final boolean isAnimating = (animation != Animation.NONE);
    
    // The large hint is drawn by Android, so do nothing.
    if (!isAnimating
        && TextUtils.isEmpty(getText())) { return; }
    
    final Paint paint = getPaint();
    final float hintPosX = getCompoundPaddingLeft()
        + getScrollX();
    final float normalHintPosY = getBaseline();
    final float floatingHintPosY = normalHintPosY
        + paint.getFontMetricsInt().top
        + getScrollY();
    final float normalHintSize = getTextSize();
    final float floatingHintSize = normalHintSize
        * HINT_SCALE;
    ...
    // If we're not animating, we're showing the floating hint, so draw it and
    // bail.
    if (!isAnimating) {
    ...
    if (animation == Animation.SHRINK) {
      drawAnimationFrame(canvas, normalHintSize, floatingHintSize, hintPosX, normalHintPosY, floatingHintPosY, normalHintColor, floatingHintColor);
    } else {
      drawAnimationFrame(canvas, floatingHintSize, normalHintSize, hintPosX, floatingHintPosY, normalHintPosY, floatingHintColor, normalHintColor);
    }
    
    animationFrame++;
    
    if (animationFrame == ANIMATION_STEPS) {
      // After the grow animation has finished, restore the normal TextView hint
      // color that we
  ...
  private void drawAnimationFrame(
      Canvas canvas, float fromSize,
      float toSize, float hintPosX,
      float fromY, float toY,
      int fromColor, int toColor) {
    final float textSize = lerp(fromSize, toSize);
    final float hintPosY = lerp(fromY, toY);
    final int color = Color.rgb((int) lerp(Color.red(fromColor), Color.red(toColor)), (int) lerp(Color.green(fromColor), Color.green(toColor)), (int) lerp(Color.blue(fromColor), Color.blue(toColor)));
  ...
  private float lerp(float from,
      float to) {
    final float alpha = (float) animationFrame
        / (ANIMATION_STEPS - 1);
    return from * (1 - alpha) + to
        * alpha;
  }
}
